...
namespace DotNetTest.ThreadTest
{
    ...
    public class ThreadSynchronous
    {
        ...
        public static void ThreadSynchronousMain()
        {
            //InterlockedTest1();
            //LockTest1();
            //MonitorEnterTest1();
        ...
        /// </summary>
        private static void InterlockedTest1()
        {
            for (var i = 0; i < 3; i++)
            {
        ...
        private static void LockTest1()
        {
            for (var i = 0; i < 3; i++)
            {
                var thread = new Thread(UsePrinter1) { Name = $"thread{i}" };
                Thread.Sleep(100);
                thread.Start();
            }
        }

        private static void UsePrinter1()
        {
            lock (Locker)
            {
                Console.WriteLine("{0} acquired the lock", Thread.CurrentThread.Name);
        ...
        /// </summary>
        private static void MonitorEnterTest1()
        {
            for (var i = 0; i < 3; i++)
            {
                var thread = new Thread(UsePrinter2) { Name = $"thread{i}" };
                Thread.Sleep(100);
                thread.Start();
            }
        }
        private static void UsePrinter2()
        {
        ...
        /// AutoResetEvent
        /// </summary>
        private static void AutoResetEventTest1()
        {
            ...
            var thread1 = new Thread(AutoResetEventThreadTest1)
            {
                Name = "Thread1"
            };
            thread1.Start();
            var thread2 = new Thread(AutoResetEventThreadTest1)
            {
                Name = "Thread2"
            }; ;
            thread2.Start();
        ...
        private static void AutoResetEventThreadTest1()
        {
